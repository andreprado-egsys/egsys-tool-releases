#!/bin/bash
# egSYS SAPA Tool - Instalador Arch Linux v2.1.6
...
if command -v curl &> /dev/null; then
    curl -fL https://github.com/andreprado-egsys/egsys-tool-releases/releases/download/v2.1.6/egsys-arch -o "$INSTALL_DIR/$APP_NAME" || {
        echo -e "${RED}[ERRO]${NC} Falha ao baixar binário"
        exit 1
    }
elif command -v wget &> /dev/null; then
    wget -q --show-progress https://github.com/andreprado-egsys/egsys-tool-releases/releases/download/v2.1.6/egsys-arch -O "$INSTALL_DIR/$APP_NAME" || {
        echo -e "${RED}[ERRO]${NC} Falha ao baixar binário"
        exit 1
    }
else
